...
enum UUNavigationBarStyle {
    case theme
    case clear
    case white
}

class UUNavigationController: UINavigationController {

    override func viewDidLoad() {
        super.viewDidLoad()

        guard let intercationGes = interactivePopGestureRecognizer else { return }
        guard let targetView = intercationGes.view else { return }
        guard let internalTargets = intercationGes.value(forKeyPath: "targets") as? [NSObject] else { return }
        ...
        intercationGes.isEnabled = false
    }
...
extension UUNavigationController: UIGestureRecognizerDelegate {
...
extension UUNavigationController {
...
extension UINavigationController {
    ...
    func barStyle(_ style: UUNavigationBarStyle) {
        switch style {
        case .theme:
